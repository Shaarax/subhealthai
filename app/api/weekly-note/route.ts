...
if (!user_id) {
  ...
  if (apiKey) {
    try {
      ...
      const bullets = (flags ?? []).map(f => `- ${f.flag_type} (sev ${f.severity}) — ${f.rationale}`).join('\n') || '- none -'
      const prompt = `Generate a weekly preventive health note (not diagnostic).
Ground ONLY in these signals.
Totals: ${JSON.stringify(counts, null, 0)}
Details:
${bullets}`
      ...
      note_json.interpretation.text = resp.choices?.[0]?.message?.content?.trim() || ''
    } catch {
