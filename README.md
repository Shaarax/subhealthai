...
- **Copilot data layer** — *known functional regression.* The copilot retrieves data through cookieless server-to-server self-fetches of the (now session-gated) read routes (`lib/copilot/tools.ts`, `lib/copilot/toolsRunner.ts`, `app/api/copilot/llm/route.ts`). With authorization fixed, those self-fetches fail for real users, so the real-user copilot currently errors while the demo copilot still works. The fix is to call the data-building functions directly (as done in `app/api/report/route.ts`), which first requires extracting the read routes' inline logic into shared libs — a dedicated refactor, not yet done.
